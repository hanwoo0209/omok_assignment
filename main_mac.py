...
canvas.create_line(50, 50, 50, 650, 650, 650, 650, 50, 50, 50, width=3, fill="black") # 아웃라인
a=100 # 가로선 초기값
b=100 # 세로선 초기값
c=50 # 가로숫자 초기값
d=50 # 세로숫자 초기값
for i in range(11): # 가로선
    canvas.create_line(a, 50, a, 650, width=1, fill="black")
    a+=50
for i in range(11): # 세로선
    canvas.create_line(50, b, 650, b, width=1, fill="black")
    b+=50
...
listbox.config(fg="white", font="맑은고딕 25")
...
lb1=Label(frame_right, width=5, height=1, font="맑은고딕 16", text="가로")
lb1.place(x=25, y=465)
entry_y=Entry(frame_right, width=7, font="맑은고딕 17") #세로 엔트리
entry_y.place(x=85, y=500)
lb2=Label(frame_right, width=5, height=1, font="맑은고딕 16", text="세로")
...
button_back=Button(frame_right, width=9, height=1, font="맑은고딕 16")
button_back.config(text="지우기", command=take_erase_turns)
button_back.place(x=25, y=397)

button_start=Button(frame_right, width=5, height=3, font="맑은고딕 17")
button_start.config(text="착수", command=take_turns)
button_start.place(x=180, y=465)

button_close=Button(frame_right, width=7, height=1, font="맑은고딕 17")
button_close.config(text="종료", command=return_close)
button_close.place(x=25, y=645)

button_restart=Button(frame_right, width=7, height=1, font="맑은고딕 17")
